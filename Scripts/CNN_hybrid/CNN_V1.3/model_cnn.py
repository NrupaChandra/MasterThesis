...
class NodalPreprocessor(nn.Module):
    def __init__(self, num_nodes=250000, domain=(-1, 1)):
...
class CNN_FNN(nn.Module):
    def __init__(self, hidden_dim, output_dim, max_output_len, num_nodes=250000, domain=(-1,1), dropout_rate=0.0):
        super(CNN_FNN, self).__init__()
        self.nodal_preprocessor = NodalPreprocessor(num_nodes=num_nodes, domain=domain)
        self.grid_size = int(np.sqrt(num_nodes))  # e.g., 35 for 1225 nodes
        
        # - Increase depth by stacking  conv layers.
        # - Increase width: first conv outputs 16 channels, second outputs 32.
        # - Batch normalization is incorporated in each block.
        self.conv_layers = nn.Sequential(
            ConvBlock(1, 8),   # First conv block with dropout
            #nn.MaxPool2d(kernel_size=2),
        )
            
        # Flattened feature dimension 
        pooled_size = self.grid_size //( 1 * 1 )
        flattened_dim = 8 * pooled_size * pooled_size
    ...
    def forward(self, exp_x, exp_y, coeff):
        ...
        # Apply the multi-layer convolutional network.
        conv_out = self.conv_layers(nodal_image)
...
def load_shallow_cnn_model(weights_path=None, hidden_dim=256, output_dim=256, max_output_len=16,
                           num_nodes=250000, domain=(-1,1), dropout_rate=0.0):
...
model = load_shallow_cnn_model(hidden_dim=256, output_dim=256, max_output_len=16,
                               num_nodes=250000, domain=(-1,1), dropout_rate=0.0)
