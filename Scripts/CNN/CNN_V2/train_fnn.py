...
model_dir = "/work/scratch/ng66sume/MasterThesis/Models/CNN/CNN_V2/"
...
def train_cnn(model, train_dataloader, val_dataloader, optimizer, criterion, test_functions, 
              num_nodes, domain, epochs=1000, checkpoint_path=None, save_every=100):
    ...
    final_model_path = os.path.join(model_dir, 'cnn_model_weights_v2.0.pth')
...
if __name__ == "__main__":
    ...
    dataset = MultiChunkDataset(
        index_file=os.path.join(data_dir, 'preprocessed_chunks_1Mill/index.txt'),
        base_dir=data_dir
    )
    ...
    model = load_shallow_cnn_model(num_nodes=122500, domain=(-1,1), dropout_rate=0.0)
    model.to(device)
    ...
    epoch_list, train_losses, val_losses, epoch_times = train_cnn(
        ...
        num_nodes=122500, domain=(-1,1),
        epochs=epochs, checkpoint_path=os.path.join(model_dir, "cnn_checkpoint.pth"), save_every=100
    )
