...
model_dir = "/work/scratch/ng66sume/Models/CNN/CNN_V3/"
results_dir = "/work/scratch/ng66sume/Test_Results/CNN/CNN_V3/"
...
with torch.no_grad():
    for sample in dataloader:
        ...
        predicted_weights_tensor = model(exp_x, exp_y, coeff)  # shape: (batch, 1, 16, 16)
        batch_size = predicted_weights_tensor.size(0)

        # Instead of using the nodal preprocessor's grid, generate a fixed 16x16 grid.
        ...
        predicted_nodes_x_tensor = X_pred.flatten().unsqueeze(0).expand(batch_size, -1)
        predicted_nodes_y_tensor = Y_pred.flatten().unsqueeze(0).expand(batch_size, -1)
