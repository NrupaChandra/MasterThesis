...
class NodalPreprocessor(nn.Module):
    def __init__(self, num_nodes=25, domain=(-1, 1)):
        ...
        self.register_buffer("X", X.flatten())  #register_buffer is  a way to store a tensor inside a PyTorch model without treating it as a trainable parameter.
...
class FeedForwardNN(nn.Module):
    def __init__(self, hidden_dim, output_dim, max_output_len, num_residual=6, dropout=0.1, num_nodes=25, domain=(-1,1)):
...
def load_ff_pipelines_model(weights_path=None, hidden_dim=1024, output_dim=1024, max_output_len=16,
                            num_residual=6, dropout=0.1, num_nodes=25, domain=(-1,1)):
    model = FeedForwardNN(hidden_dim, output_dim, max_output_len, num_residual, dropout, num_nodes, domain)
    model = model.double()
    if weights_path:
