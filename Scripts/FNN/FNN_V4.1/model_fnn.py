...
class FeedForwardNN(nn.Module):
    def __init__(self, hidden_dim, output_dim, max_output_len, num_nodes=25, domain=(-1,1)):
        ...
        self.x_layer = nn.Sequential(
            nn.Linear(input_dim, max_output_len),
            nn.Tanh()
        )
    ...
    def forward(self, exp_x, exp_y, coeff):
        ...
        pred_nodes_x = self.x_layer(x) # torch.tanh(self.node_x_head(nodes_out))
        pred_nodes_y = self.y_layer(x) # torch.tanh(self.node_y_head(nodes_out))
        
        # weights_out =  self.weights_branch(x)
        pred_weights = self.w_layer(x) # F.softplus(self.weight_head(weights_out))
