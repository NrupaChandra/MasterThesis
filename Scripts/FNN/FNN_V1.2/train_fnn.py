...
model_dir = "/work/home/ng66sume/MasterThesis/Models/FNN_model_v1.2/"
...
def custom_collate_fn(batch):
    ...
    padded_nodes_x = torch.stack([
        torch.cat([x, torch.zeros(max_len - x.size(0), dtype=torch.float64)])
        for x in nodes_x
    ])
    padded_nodes_y = torch.stack([
        torch.cat([y, torch.zeros(max_len - y.size(0), dtype=torch.float64)])
        for y in nodes_y
    ])
    padded_weights = torch.stack([
        torch.cat([w, torch.zeros(max_len - w.size(0), dtype=torch.float64)])
        for w in weights
    ])
    masks = torch.stack([
        torch.cat([torch.ones(x.size(0), dtype=torch.float64),
                   torch.zeros(max_len - x.size(0), dtype=torch.float64)])
        for x in nodes_x
    ])
...
def train_fnn(model, train_dataloader, val_dataloader, optimizer, criterion, test_functions, 
    ...
    final_model_path = os.path.join(model_dir, 'fnn_model_weights_v1.2.pth')
