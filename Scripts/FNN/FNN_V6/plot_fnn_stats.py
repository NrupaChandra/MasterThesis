...
file_path = r"C:\Git\RESULTS\Fnn\console_output.txt"

# ----------------------------
# 3. Parse the Console Output
# ----------------------------
true_pattern      = re.compile(r"Algoim \(True\):\s+([0-9.Ee+-]+)")
pred_pattern      = re.compile(r"QuadNET \(Pred\):\s+([0-9.Ee+-]+)")
...
# ----------------------------
fig1, ax1 = plt.subplots(figsize=(6, 4))  # SAME size as histogram
...
ax1.plot([minv, maxv], [minv, maxv], color='dimgray', linestyle='--', linewidth=1.0, label='Ideal (y = x)')
ax1.set_xlabel('True Integral', fontsize=12)
ax1.set_ylabel('Predicted Integral', fontsize=12)
ax1.set_title('Predicted vs True Integrals- FNN', fontsize=12)
ax1.legend(loc='upper left', fontsize=10)
ax1.grid(True, linestyle='--', linewidth=0.5)

# Inset Zoom (0–2.5)
axins = inset_axes(ax1, width="40%", height="40%", loc='lower right',
                   bbox_to_anchor=(0.05, 0.05, 0.5, 0.5),
                   bbox_transform=ax1.transAxes)

axins.scatter(df['True'], df['Pred'],
              s=20, alpha=0.6,
              color='tab:blue', edgecolors='k', linewidth=0.3)
axins.plot([0, 2.5], [0, 2.5], color='dimgray', linestyle='--', linewidth=0.8)
...
plt.tight_layout()
plt.savefig('scatter_pred_vs_true_improved_FNN.png', dpi=300)
plt.close()
...
# ----------------------------
fig4, ax4 = plt.subplots(figsize=(6, 4))  # SAME size as scatter
...
ax4.hist(df['RelErr'], bins=bins_rel, edgecolor='black', color='tab:blue', alpha=0.7)
ax4.set_xscale('log')
...
ax4.set_title('Histogram of Relative Errors- FNN', fontsize=12)
...
plt.tight_layout()
plt.savefig('hist_relative_error_log_bins_power10_blue_FNN.png', dpi=300)
plt.close()


print("All improved plots have been generated and saved:")
print("  • scatter_pred_vs_true_improved.png")
print("  • hist_relative_error_log_bins_improved.png")
