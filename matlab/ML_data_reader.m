% Load the required machine learning data
...
randomIndices_ML = randperm(length(allIDs_ML), numPlots);

% Define folder to save plots
outputFolder = 'C:\Git\MasterThesis\matlab\ml_plots';  % Updated folder path for ML plots
...
for i = 1:numPlots
    ...
    currentIndex = randomIndices_ML(i);
    currentID = allIDs_ML{currentIndex};
    
    % Extract corresponding data for nodes and weights from machine learning data
    currentNodesX_ML = nodes_x_with_id_ML{currentIndex}{2};  % Second element contains x-coordinates
    currentNodesY_ML = nodes_y_with_id_ML{currentIndex}{2};  % Second element contains y-coordinates
    currentWeights_ML = weights_with_id_ML{currentIndex}{2}; % Second element contains weights
    ...
    % Plot the scatter of nodes with weights from ML data
    scatter(currentNodesX_ML, currentNodesY_ML, 20, currentWeights_ML, 'filled', 'MarkerEdgeColor', 'k');
    colorbar;
    colormap(jet);
    
    % Customize the plot
    title(['ML plot for ID: ' currentID]);
    xlabel('x');
    ylabel('y');
    axis equal;
    xlim([-1, 1]);
    ...
    plotFileName = fullfile(outputFolder, ['ml_combined_levelset_nodes_ID_' currentID '.png']);
